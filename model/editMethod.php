...
if ($_POST['acceptParemeter'] == 0)
  {
	...
	$sql = "SELECT no FROM dataSet WHERE algorithmID=%d_algorithmID ORDER BY no desc";
            $dataSetData = $praetor->custosql($sql, array('algorithmID'=>$_POST['aid']));
            if ($dataSetData)
            {
            	$n = 0;
            	while ($n <= $dataSetData[0]['no'])
            	{
					...
					$dataSetWhere->add('algorithmID=%s', $_POST['aid']);
            ...
            $paremeterData = $praetor->custosql($sql, array('algorithmID'=>$_POST['aid']));
            if ($paremeterData)
            {
            	$n = 0;
            	while ($n <= $paremeterData[0]['no'])
            	{
					...
					$paremeterWhere->add('algorithmID=%s', $_POST['aid']);
...
$mailBody = "<div><label>您已成功修改演算法，以下為您的修改資訊：</label></div><fieldset>
      <legend>程式名稱:</legend>
	<div>
                              <label>簡寫:</label>
...
<div class="form-group">
	<form action="index.php?item=praetorLogin" method="post">
		<h3 class="Topic">您已成功修改演算法，以下為您的修改資訊：</h3>
